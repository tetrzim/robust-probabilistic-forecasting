...
parser.add_argument('--metric_path_base', type=str, default=None, help='path to .pkl file containing the Metrics type object (from base model)')
...
print(" \\\\\n".join([" & ".join(map('{0:.3f}'.format, line))
    for line in np.hstack((
        np.reshape(tolerance, (len(tolerance), 1)),
        nd_table_base[:, [0, sigma_idx]],
        nd_table_rand[:, [0, sigma_idx]]
    ))
]))
