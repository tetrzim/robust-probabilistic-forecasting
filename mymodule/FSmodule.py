...
class DeepARModel_FS(nn.Module):
    @validated()
    def __init__(
        self,
        freq: str,
    ...
    def forward(
        ...
        intermediate_noise: Optional[float] = None,
        noise_lag_index: Optional[int] = None,
    ) -> torch.Tensor:
        ...
        next_sample = distr.rsample(sample_shape=(num_parallel_samples,))
        ...
        if intermediate_noise is None:
            ...
            for k in range(1, self.prediction_length):
                ...
                if intermediate_noise is not None:
                    next_sample = next_sample + torch.normal(mean=torch.zeros_like(next_sample),
                                                            std=intermediate_noise * torch.abs(next_sample))
            ...
            sigma = intermediate_noise

        if noise_lag_index is not None:
            repeated_past_target = repeated_past_target[:, :-noise_lag_index]
            future_samples = [past_target[:, -noise_lag_index:] / scale, torch.mean(next_sample, dim=0)]
        else:
            future_samples = [torch.mean(next_sample, dim=0)]
            noise_lag_index = 0
        ...
        for k in range(1, self.prediction_length):
            ...
            last_sample = future_samples[-1].clone()
            last_sample = last_sample.repeat_interleave(
                repeats=num_parallel_samples, dim=0
            )
            last_sample = last_sample + torch.normal(
                mean=torch.zeros_like(last_sample), std=sigma
            )

            if len(future_samples) > 1:
                noised_future_samples = torch.cat(
                    future_samples[:-1], dim=1
                )
                noised_future_samples = noised_future_samples.repeat_interleave(
                    repeats=num_parallel_samples, dim=0
                )
                noised_future_samples = noised_future_samples + torch.normal(
                    mean=torch.zeros_like(noised_future_samples),
                    std=sigma
                )
                repeated_noised_input = torch.cat(
                    (repeated_past_target, noised_future_samples), dim=1
                )   
            else:
                repeated_noised_input = repeated_past_target

            output, repeated_state = self.lagged_rnn(
                repeated_noised_input,
                last_sample,
                next_features,
                repeated_state,
            )
            ...
            distr = self.output_distribution(params)
            next_sample = distr.rsample()
            next_sample = torch.transpose(
                torch.reshape(next_sample, (-1, num_parallel_samples, 1)), 0, 1
            )
            future_samples.append(torch.mean(next_sample, dim=0))

        unscaled_future_samples = (
            torch.cat(future_samples[noise_lag_index:], dim=1) * scale
        )
        
        return unscaled_future_samples.reshape(
            (-1, 1, self.prediction_length)
            + self.target_shape,
        ), scale
